...
{
    public class Op : Enumeration
    {
        // Lua 5.1 Opcodes
        public const OpcodeFormat MOVE = OpcodeFormat.A_B,
        ...
        public string codePointToString(int codepoint, ICodeExtract ex)
        {
            switch (format)
            {
                case OpcodeFormat.A: return ToString() + " " + ex.extract_A(codepoint);

                case OpcodeFormat.A_B: return ToString() + " " + ex.extract_A(codepoint) + " " + ex.extract_B(codepoint);

                case OpcodeFormat.A_C: return ToString() + " " + ex.extract_A(codepoint) + " " + ex.extract_C(codepoint);

                case OpcodeFormat.A_B_C: return ToString() + " " + ex.extract_A(codepoint) + " " + ex.extract_B(codepoint) + " " + ex.extract_C(codepoint);

                case OpcodeFormat.A_Bx: return ToString() + " " + ex.extract_A(codepoint) + " " + ex.extract_Bx(codepoint);

                case OpcodeFormat.A_sBx: return ToString() + " " + ex.extract_A(codepoint) + " " + ex.extract_sBx(codepoint);

                case OpcodeFormat.Ax: return ToString() + " <Ax>";

                case OpcodeFormat.sBx: return ToString() + " " + ex.extract_sBx(codepoint);
