...
namespace Marathon.Formats.Mesh.Ninja.Types
{
    ...
    {
        public static object ReadKeyframeByType(BinaryObjectReaderEx in_reader, SubMotion in_subMotion)
        {
            if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_TRANSLATION_MASK) ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_SCALING_MASK)     ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_AMBIENT_MASK)     ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_DIFFUSE_MASK)     ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_SPECULAR_MASK)    ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_LIGHT_COLOR_MASK))
            {
                return in_reader.Read<KeyframeVector>();
            }

            if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_ROTATION_XYZ))
                return in_reader.Read<KeyframeRotateS16>();

            if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_FRAME_FLOAT))
                return in_reader.Read<KeyframeF32>();

            if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_FRAME_SINT16))
                return in_reader.Read<KeyframeS16>();

            throw new NotImplementedException();
        }

        public static void WriteKeyframeByType(BinaryObjectWriterEx in_writer, SubMotion in_subMotion, object in_keyframe)
        {
            if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_TRANSLATION_MASK) ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_SCALING_MASK)     ||
                in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_AMBIENT_MASK)     ||
            ...
            else if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_ROTATION_XYZ))
            {
                in_writer.Write((KeyframeRotateS16)in_keyframe);
            }
            else if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_FRAME_FLOAT))
            {
                in_writer.Write((KeyframeF32)in_keyframe);
            }
            else if (in_subMotion.Type.HasFlag(SubMotionType.NND_SMOTTYPE_FRAME_SINT16))
            {
