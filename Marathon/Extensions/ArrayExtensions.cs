...
namespace Marathon.Extensions
{
    public static class ArrayExtensions
    {
        extension(Array)
        {
            ...
            public static T[][] CreateInstanceJagged2D<T>(params int[] in_lengths)
            {
                ThrowHelper.ThrowArgumentNullException(nameof(in_lengths), in_lengths);

                if (in_lengths.Length != 2)
                    throw new ArgumentException("Exactly two lengths are required.");

                return (T[][])CreateInstanceJagged<T>(in_lengths);
            }
